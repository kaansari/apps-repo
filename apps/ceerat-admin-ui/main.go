...
import (
	...
	"github.com/kaansari/ceerat-platform/apps/ceerat-admin-ui/internal/server"
)

func main() {
	cfg := config.Load()
	client, err := apiclient.New(cfg.APIBaseURL, cfg.AdminBaseURL)
	if err != nil {
	...
	go func() {
		log.Printf("ceerat-admin-ui listening on http://localhost:%s", cfg.Port)
