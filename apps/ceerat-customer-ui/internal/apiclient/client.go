...
func (c *Client) RegisterCustomer(ctx context.Context, input RegisterCustomerInput) (Session, error) {
	ctx, cancel := context.WithTimeout(ctx, 8*time.Second)
	defer cancel()

	res, err := c.auth.RegisterCustomer(ctx, &authpb.RegisterCustomerRequest{
		...
		AddressLine1: strings.TrimSpace(input.Line1),
		AddressLine2: strings.TrimSpace(input.Line2),
...
func (c *Client) Dashboard(ctx context.Context, userID string) (Dashboard, error) {
	...
	customer, err := c.GetMyCustomerProfile(ctx)
	if err != nil {
	...
	return Dashboard{
		...
		Services:         services,
		CustomerServices: nil,
	}, nil
}

func (c *Client) GetMyCustomerProfile(ctx context.Context) (Customer, error) {
	ctx, cancel := context.WithTimeout(ctx, 8*time.Second)
	defer cancel()
	ctx = outgoingAuthContext(ctx)
	res, err := c.customers.GetMyCustomerProfile(ctx, &customerpb.GetMyCustomerProfileRequest{})
	if err != nil {
		return Customer{}, err
	}
...
func (c *Client) UpdateMyCustomerProfile(ctx context.Context, customer Customer) (Customer, error) {
	ctx, cancel := context.WithTimeout(ctx, 8*time.Second)
	defer cancel()
	ctx = outgoingAuthContext(ctx)
	res, err := c.customers.UpdateMyCustomerProfile(ctx, &customerpb.UpdateMyCustomerProfileRequest{
		...
		Address: &customerpb.Address{
			Line1:      strings.TrimSpace(customer.Address.Line1),
			Line2:      strings.TrimSpace(customer.Address.Line2),
			City:       strings.TrimSpace(customer.Address.City),
			State:      strings.TrimSpace(customer.Address.State),
			Country:    strings.TrimSpace(customer.Address.Country),
			PostalCode: strings.TrimSpace(customer.Address.PostalCode),
		},
	})
	if err != nil {
		return Customer{}, err
...
func (c *Client) CreateOrder(ctx context.Context, userID string, input CreateOrderInput) (Order, error) {
	...
	res, err := c.orders.CreateMyOrder(ctx, &orderpb.CreateMyOrderRequest{
...
func (c *Client) ListOrders(ctx context.Context, userID, customerID, status string) ([]Order, error) {
	...
	res, err := c.orders.ListMyOrders(ctx, &orderpb.ListMyOrdersRequest{Status: strings.TrimSpace(status), PageSize: 100})
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetOrder(ctx context.Context, userID, id string) (Order, error) {
	...
	res, err := c.orders.GetMyOrder(ctx, &orderpb.GetMyOrderRequest{Id: strings.TrimSpace(id)})
	if err != nil {
		return Order{}, err
	}
	return orderFromProto(res.GetOrder()), nil
