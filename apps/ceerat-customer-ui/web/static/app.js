...
document.querySelectorAll("[data-auth-form]").forEach((form) => {
  form.addEventListener("submit", async (event) => {
    ...
    if (data.confirmPassword !== undefined && data.password !== data.confirmPassword) {
      setMessage(form, "Password and confirmation must match.");
      return;
    }
    ...
    try {
      ...
      await postJSON(form.dataset.endpoint, data);
...
function renderAvailableServices() {
  const list = document.querySelector("[data-available-services-list]");
  if (!list) return;
  list.replaceChildren();
  if (!dashboardState.services.length) {
    const row = document.createElement("tr");
    row.innerHTML = `<td colspan="3">No services available yet.</td>`;
    list.append(row);
    return;
  }
  dashboardState.services.forEach((service) => {
    ...
    category.textContent = service.category || service.type || "";
    const price = document.createElement("td");
    price.textContent = formatMoney(service.price);
    row.append(name, category, price);
    list.append(row);
  });
}
...
document.querySelectorAll("[data-customer-form]").forEach((form) => {
  form.addEventListener("submit", async (event) => {
    ...
    const data = formData(form);
    const endpoint = "/api/customer/me";
    const button = form.querySelector("button[type='submit']");
    setButtonBusy(button, true);
    setMessage(form, "Saving...", "neutral");

    try {
      await requestJSON(endpoint, "PATCH", data);
      setMessage(form, "Profile saved.", "success");
      resetCustomerForm();
      await loadDashboard();
    } catch (error) {
      setMessage(form, error.message);
